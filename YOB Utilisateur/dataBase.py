...
def loginDatabaseClient(Email,Password):
    cursor.execute("SELECT idClient,emailAdress,password FROM client")
    i=0
    resultats = cursor.fetchall()
    for x in resultats:
        if (x[1],x[2])==(Email,Password):
            i=x[0]
            break
    return i
#Supprimer un Client
...
def changeInformation(idClient,nvNumeroTelephone,nvPassword):
    ...
    conn.commit()
def setSolde(idClient,solde):
    cursor.execute("UPDATE client SET solde=%s WHERE idClient=%s", (float(solde), idClient))
    conn.commit()
#return data from id
def returnDataApartirId(id):
    cursor.execute("SELECT idClient,nom,prenom,age,cin,emailAdress,telephone,ville,sexe,rib,solde,password,date_Creation FROM client WHERE idClient=%s",(id,))
    resultats=cursor.fetchall()
    for x in resultats:
        return x
#Email Existe ?
